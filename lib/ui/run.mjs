...
import { InitView, CheckView, TestView, ConformanceView, ErrorView, HelpView } from "./views.mjs";
import { initFlow, snapshotFlow, checkFlow } from "../commands.mjs";
import { testFlow } from "../test-runner.mjs";
import { conformanceFlow } from "../conformance.mjs";
...
export async function runConformance(cwd, version, { strict = false, targetOverride = null } = {}) {
  const report = await withProgress(
    ConformanceView,
    (onPhase) => conformanceFlow(cwd, version, { strict, targetOverride, onPhase }),
    (r) => ({ report: r })
  );
  return report.exitCode;
}

/** check --all: drift check, then response tests, then (opt-in) conformance. Exit is the worst. */
export async function runCheckAll(cwd, version, failOn, targetOverride, hasTests, conformance = false) {
  let exit = await runCheck(cwd, version, failOn, targetOverride);
  if (hasTests) exit = Math.max(exit, await runTest(cwd, version, { targetOverride }));
  if (conformance) exit = Math.max(exit, await runConformance(cwd, version, { targetOverride }));
  return exit;
}
