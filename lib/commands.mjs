...
export async function checkFlow(cwd, version, failOn, onPhase, targetOverride = null) {
  // CI often needs a different endpoint than the committed config (e.g. a
  // server started on localhost in the workflow) — --url/--stdio override
  // the configured target without touching the config file.
  const target = targetOverride ?? loadConfig(cwd).target;
